...
from HiMAGE.agent.agent import IMGEPAgent
from HiMAGE.hyper_parameters import EXPLOIT_PROB, HORIZON, LAYOUT_ID, LOAD_KB, ROLLOUTS
from HiMAGE.visualise.create_gif import create_gif
from HiMAGE.visualise.visualise import make_graphs
...
for roll in range(ROLLOUTS):
    ...
    for ag in agents: ag.reset(mdp, exploit=exploit, rollout_idx=roll)
    done = False
    state = env.state
    # -------- record trajectory -----------------------------------------
