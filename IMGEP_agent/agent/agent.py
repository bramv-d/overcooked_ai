...
class IMGEPAgent(Agent):
    ...
    def reset(self, mdp: OvercookedGridworld | None = None):
        ...
        self.rollout_fitness = 0.0
        self.previous_state = None
        self.exploit = random.random() < EXPLOIT_PROB
        chosen_goal = select_goal_space(all_goal_spaces=self.goal_spaces, kb=self.kb)
        neuro_policy = get_neuro_policy(selected_goal=chosen_goal, kb=self.kb, exploit=self.exploit)
        self.goal_space_neuro_policies = [
            GoalSpaceNeuroPolicy(goal=chosen_goal, neuro_policy=neuro_policy)
    ...
    def action(self, state: OvercookedState) -> Action:
        ...
        token = HighLevelActions(
            self.goal_space_neuro_policies[-1].neuro_policy.select_token(obs_vec, greedy=greedy))
    ...
    def finish_rollout(self, final_state: OvercookedState, info):
        ...
        prev_record = self.kb.nearest(goal, 1, exploit=self.exploit)
